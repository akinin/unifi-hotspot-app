...
- Верхняя сводка Hotspot теперь остаётся на месте при переходе между активными клиентами и архивом.
- Имена клиентов, полученные из UniFi, сохраняются в истории авторизаций; MAC-адрес больше не дублируется вместо имени.

## 0.6.6

- Приложение переведено из экспериментальной стадии в стабильную.
- Карточка Home Assistant получила русское краткое описание.
- Информационная страница дополнена возможностями, требованиями, разделами интерфейса и порядком первого запуска.

## 0.6.5

- Репозиторий, документация и контейнерный образ переименованы в `unifi-hotspot-app` в соответствии с текущим назначением проекта.
- Внутренний slug сохранён для бесшовного обновления существующей установки и сохранения её данных.

## 0.6.4

- Кнопки «Отозвать» и «Блокировать» снова расположены рядом и больше не наследуют четырёхколоночную сетку кнопок продления.
- Последний известный IP клиента сохраняется локально и остаётся видимым, пока UniFi не сообщит новый адрес.

## 0.6.3

- В активные клиенты добавлены уже авторизованные и подключённые устройства из UniFi, даже если они входили не через этот портал.
- Для импортированных клиентов доступны продление, отзыв и блокировка; телефон показывается только для входов через SMS-портал.
- Исправлено компактное меню действий: опасные действия больше не обрезаются на узких экранах.
- Чтение клиентов UniFi кэшируется и объединяется, чтобы не создавать лишние входы в контроллер и ошибки ограничения запросов.

## 0.6.2

- Галочка успешного уведомления заменена на контрастную SVG-пиктограмму.
- Из первой секции портала убраны заголовок и сводка оформления; оставлена только компактная кнопка предпросмотра.

## 0.6.1

- Исправлен контраст пиктограмм в уведомлениях об успешной отправке и ошибках.

## 0.6.0

- Конфигурация разделена на компактные логические группы.
- Добавлена автоматическая миграция прежних плоских настроек без потери значений.
- Документация переписана как пошаговое руководство по WB, USB, UniFi и порталу.
- Для SMS-транспорта разрешены только поддерживаемые значения `mqtt` и `mmcli`.
