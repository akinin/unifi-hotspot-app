...
В репозитории есть экспериментальная Home Assistant App-версия для безопасного
параллельного запуска рядом с действующим LXC. По умолчанию она использует
отдельные порты, отдельную базу и режим UniFi dry-run. Инструкции находятся в
[`home-assistant-app/DOCS.md`](home-assistant-app/DOCS.md).
